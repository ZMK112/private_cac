...
_dk_cmd_setup() {
  ...
  local proxy mode detected_mode docker_dir data_dir container_name runtime_hostname gateway_name child_proxy child_no_proxy
  proxy=$(_dk_prompt_value "Proxy URI (e.g. ss://..., ip:port:user:pass)" "$(_dk_read_env PROXY_URI)" 1) || return 1
  ...
  child_proxy="${child_proxy:-$(_dk_guess_child_proxy_url "$mode" "$proxy")}"
...
_dk_cmd_rebuild() {
  ...
  echo ""
  if [[ "$state" == "running" ]]; then
    _warn "Container is still using the previous image until it is restarted"
    _info "Next: \033[1mcac docker restart\033[0m"
  else
