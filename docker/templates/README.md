...
- `docker/Dockerfile`, which copies `docker/templates` into `/usr/local/share/cherny`
- `docker/entrypoint.sh`, which reads the `cherny.*` files to build the canonical identity, env, prompt, and telemetry profile inside the container
