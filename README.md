...
如果你希望别人**直接从 GitHub 下载压缩包、解压后就能安装**，正确方式是：

- 用 `./scripts/package-portable.sh` 在本地生成压缩包
- 把生成的 `dist/cac-portable-latest.zip` 上传到 GitHub Releases 的 asset
- 让别人从 GitHub Releases 下载后解压执行 `bash install.sh`
